...
// GetDescriptionParams defines the parameters for the cityTime tool.
type GetCityDescription struct {
	City string `json:"city" jsonschema:"City to get time for (nyc, sf, or boston)"`

}


func getCityDescription(ctx context.Context, req *mcp.CallToolRequest, params *GetCityDescription) (*mcp.CallToolResult, any, error) {
	...
	description := map[string]string{
		"nyc":    "The weather in New York City is currently sunny.",
		"sf":     "The weather in San Francisco is currently rainy.",
		"boston": "The weather in Boston is currently snowy.",
	}

	response := fmt.Sprintf("The description of the city %s is %s",
		cityNames[city],
		description[city])
...
func runServer(url string) {
	...
	// Add the description city
	mcp.AddTool(server, &mcp.Tool{
		Name:        "cityTime",
		Description: "Get description of the cities NYC, San Francisco, or Boston",
		InputSchema: map[string]any{
			"type": "object",
			"properties": map[string]any{
			"city": map[string]any{
				"type":        "string",
				"description": "City to get description for (nyc, sf, or boston)",
			},
		},
			"additionalProperties": true,
		},
